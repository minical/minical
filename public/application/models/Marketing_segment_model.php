...
class Marketing_segment_model extends CI_Model {
    ...
    {
        $data = (object)$data;
        $this->db->insert("marketing_segments", $data);
    ...
    function update_data($data, $debtor_id)
    {
        $this->db->where('id', $debtor_id);
        $this->db->where('company_id', $data['company_id']);
        $this->db->update("marketing_segments", $data);
    }
    ...
    function get_token($channex_id = null, $company_id = null, $ota_key = null){
    	$this->db->from('ota_manager as om');

        if($channex_id)
    	   $this->db->where('om.id', $channex_id);

        if($company_id)
           $this->db->where('om.company_id', $company_id);

       if($ota_key){
            $this->db->join('otas as o','om.ota_id = o.id', 'left');
	...
	function get_found_rows()
    {
	 ...
	 function get_debtors($filters){
        $this->db->from('marketing_segments');
    ...
    function get_debtor_data($company_id, $debtor_id){
        $this->db->select('om.*');
        $this->db->from('marketing_segments as om');
...
//echo $this->db->last_query();
    ...
    function get_channex_x_company($property_id = null, $company_id = null, $ota_key = null){
        ...
        if($property_id)
            $this->db->where('oxc.ota_property_id', $property_id);
    ...
    {
        $this->db->from('ota_room_types as ort');      
        $this->db->join('ota_x_company as oxc','ort.ota_x_company_id = oxc.ota_x_company_id AND ort.company_id = oxc.company_id');      
        $this->db->where('oxc.is_active = 1');
        $this->db->where('oxc.ota_manager_id', $ota_manager_id);
        $this->db->where('oxc.company_id', $company_id);
        $this->db->where('ort.company_id', $company_id);
    ...
    function get_channex_room_types_by_id($room_type_id = null, $company_id = null, $ota_x_company_id = null)
    {
    ...
    function get_channex_rate_plans_by_id($rate_plan_id = null, $company_id = null, $ota_x_company_id = null)
    {
    ...
    function deconfigure_channex($ota_manager_id){
