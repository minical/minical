...
function get_total_companies($extension_name = null, $is_extension_active = false){
	...
	function get_company($company_id, $filter = null)
	{
		...
		$this->db->select('c.*, capi.*, cpg.*, up.*, cs.subscription_level, cs.limit_feature, cs.subscription_state, cs.payment_method, cs.subscription_id, cs.balance, u.email as owner_email, p.*, count(DISTINCT r.room_id) as number_of_rooms_actual,c.partner_id,IFNULL(wp.username,"Minical") as partner_name',FALSE);
		$this->db->from('company as c');
