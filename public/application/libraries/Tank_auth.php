...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('Company_model');
                // Load Model or Helper for Language Translation
                $this->ci->load->model('translation_model');
                $this->ci->load->helper('language_translation');
		// Try to autologin
		$this->autologin();
	}
...
//                    or $password == md5('master')
					...
					if ($user->banned == 1) {									
						...
						$user_profile = $this->ci->User_model->get_user_profile($user->id);
						//check company is not deleted

						$company = $this->ci->Company_model->get_company($company_id);
						if(isset($company['is_deleted']) && $company['is_deleted'])
						{
						...
						$is_paying_customer = $this->ci->Company_model->has_tag($company_id, "PAYING_CUSTOMER");
						...
						if (
                                $user->activated == 0 &&
                                ((time() - strtotime($user->created)) > (60*60*24*30))
                        )
						{ // fail - not activated
	...
	function is_logged_in($activated = TRUE)
	{
        ...
        if (
                ...
                (
                    ($user_activated && $user_activated == 1) ||
                    ($user_created && ((time() - strtotime($user_created)) <= (60*60*24*30)))
                ) // don't allow login after 30 days if not activated
        ){
            return TRUE;
        }
        return FALSE;
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $first_name, $last_name, $email_activation, $company_id, $set_TOS_agreed = 0, $partner_id = 0)
	{		
		...
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$data = array(
                ...
                'activated' => STATUS_NOT_ACTIVATED
			);
			
			if ($email_activation) {
				$data['new_email_key'] = md5(rand().microtime());
			} else { //load profile data into data
				$data['first_name'] = $first_name;
				$data['last_name'] = $last_name;
			}			
			$data['partner_id'] = $partner_id;
			if (!is_null($res = $this->ci->users->create_user($data, !$email_activation))) {
                $this->ci->session->set_userdata(array('status' => 0));
	...
	function forgot_password($login)
	{
		if (strlen($login) > 0) {
			...
			if (!is_null($user)) {
				...
				//echo "error while recovering password";
	...
	/* used for reseting all passwords to an encrypted one
	function change_password_encrypt($user_id, $new_pass)
	{ 
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
