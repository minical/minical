...
class Marketing_segments extends MY_Controller
{
	...
	function __construct()
	{
        ...
        $this->load->model('Marketing_segment_model');
...
function segment_list() {
		...
		$data['rows'] = $this->Marketing_segment_model->get_debtors($filters);
        $config['total_rows'] = $this->Marketing_segment_model->get_found_rows();
	...
	{
		$data['company_id'] = $this->company_id;
        $data['debtor'] = $this->Marketing_segment_model->get_debtor_data($this->company_id, $segment_id);
	...
	function save_segment(){
		...
		if(isset($response) && $response){
			...
			$segment_data = $this->Marketing_segment_model->get_debtor_data($this->company_id, $segment_id);

			if($segment_data){
				//unset($data['email']);
				//unset($data['password']);
				$this->Marketing_segment_model->update_data($meta, $segment_id);
				$debtor_id = $segment_data['id'];
			} else {
				$debtor_id = $this->Marketing_segment_model->save_data($meta);
			}
